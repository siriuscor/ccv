...
const SAMPLE_SETTING_PATH = './setting-sample.json';

async function getSetting() {
    if (!await fs.exists(SETTING_PATH)) {
        return null;
    }
    return require(SETTING_PATH);
}

async function setSetting(setting) {
    let s = await getSetting();
    if (!s) s = require(SAMPLE_SETTING_PATH);

    let merge = {...s, ...setting};
    await fs.writeFile(SETTING_PATH, JSON.stringify(merge, null, 4));
    return merge;
}
